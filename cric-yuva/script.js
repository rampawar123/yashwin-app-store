document.addEventListener("DOMContentLoaded", function () {
  ...
  // GitHub Pages is a static host and cannot handle POST /api/* requests.
  // Keep a safe browser-only OTP fallback for development/testing so the
  // registration and OTP UI remains usable when the API is not deployed.
  const LOCAL_OTP_PREFIX = "cricYuva_local_otp_";
  function localOtpKey(mobile, purpose) {
    return LOCAL_OTP_PREFIX + String(purpose || "login") + "_" + String(mobile || "").trim();
  }
  function createLocalOtp(mobile, purpose) {
    const code = String(Math.floor(100000 + Math.random() * 900000));
    const record = { code, expiresAt: Date.now() + 5 * 60 * 1000, attempts: 0, used: false };
    try { localStorage.setItem(localOtpKey(mobile, purpose), JSON.stringify(record)); } catch (e) {}
    return code;
  }
  function verifyLocalOtp(mobile, code, purpose) {
    let record = null;
    try { record = JSON.parse(localStorage.getItem(localOtpKey(mobile, purpose)) || "null"); } catch (e) {}
    if (!record) throw new Error("No OTP found. Please request a new OTP.");
    if (record.used) throw new Error("This OTP has already been used. Please request a new OTP.");
    if (Date.now() > Number(record.expiresAt || 0)) throw new Error("OTP has expired. Please request a new OTP.");
    if (Number(record.attempts || 0) >= 5) throw new Error("Maximum verification attempts exceeded. Please request a new OTP.");
  ...
  async function requestRegistrationOtp() {
    ...
    try {
      ...
      if (isStaticApiError(e)) {
        const devOtp = createLocalOtp(mobileValue, "register");
        registerOtpRequested = true;
        if (registerOtpRow) registerOtpRow.style.display = "flex";
        if (requestRegisterOtpButton) requestRegisterOtpButton.style.display = "flex";
        if (registerOtp) registerOtp.focus();
  ...
  if (saveAccountButton) {
    saveAccountButton.addEventListener("click", async function () {
      ...
      try {
        ...
        try {
          cloud = await CricYuvaCloud.verifyOtp({ mobile: mobileValue, code: otpValue, purpose: "register", name: nameValue, password: passwordValue });
        } catch (e) {
          if (!isStaticApiError(e)) throw e;
          verifyLocalOtp(mobileValue, otpValue, "register");
          usedLocalAuth = true;
          cloud = { user: { userId: "CYU-" + mobileValue, mobile: mobileValue, name: nameValue, playerId: "CY-" + mobileValue }, token: "local-" + mobileValue };
        }
        ...
        if (window.CricYuvaStorage) {
          ...
          if (!regRes.success) {
            if (/already registered/i.test(regRes.error || "")) throw new Error(regRes.error);
            throw new Error(regRes.error || "Could not save local account.");
          }
          if (usedLocalAuth && regRes.user?.playerId) localStorage.setItem("cricYuvaPlayerId", regRes.user.playerId);
        }
  ...
  if (requestLoginOtpButton) requestLoginOtpButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (isStaticApiError(e)) {
        const devOtp = createLocalOtp(mob, "login");
        if (loginOtpCodeGroup) loginOtpCodeGroup.style.display = "block";
        if (verifyLoginOtpButton) verifyLoginOtpButton.style.display = "flex";
        if (otpDevHint) { otpDevHint.textContent = "TEST OTP: " + devOtp + " (API offline)"; otpDevHint.style.display = "block"; }
        if (loginOtpCode) loginOtpCode.focus();
        return;
      }
      alert(e.message || "OTP service unavailable.");
    }
  });
  if (verifyLoginOtpButton) verifyLoginOtpButton.addEventListener("click", async () => {
    ...
    try {
      let cloud = null;
      try {
        cloud = await CricYuvaCloud.verifyOtp({ mobile: mob, code, purpose: "login" });
      } catch (e) {
        if (!isStaticApiError(e)) throw e;
...
let auctionCloudPlayersCache = []; async function loadAuctionCloudPlayers() { try { const d = await CricYuvaCloud.request("/api/players/search"); auctionCloudPlayersCache = (d.players || []).map(p => ({ id:p.id, name:p.name, role:p.role || "All-Rounder", mobile:p.mobile || "", basePrice:Number(p.basePrice) || 1.0, avatar:p.photoUrl || p.photo_url || "🏏", type:"Cloud Player", jersey:p.jerseyNumber || p.jersey_number || 7 })); } catch(e) { console.warn("Auction cloud players load failed:",e); } } function getMasterPlayerDatabase() {
  ...
  try {
    localRegisteredPlayers("").forEach(p => {
      const pid = String(p.playerId || p.userId || p.id || "").trim();
      if (!pid) return;
      if (!masterMap.has(pid)) {
        masterMap.set(pid, {
          id: pid, playerId: pid, userId: p.userId || "",
          name: p.name || "Registered Player",
          role: p.role || "All-Rounder",
          mobile: p.mobile || "",
          basePrice: Number(p.basePrice) || 1.0,
          avatar: p.avatar || p.photoUrl || "🏏",
          type: "Registered User",
          jersey: p.jerseyNumber || 7
        });
      }
    });
  } catch (e) {
    console.warn("Could not load local registered users into auction master DB:", e);
  }

  // 5. Cloud registered players remain authoritative when available.
...
if (window.RealtimeLiveService) RealtimeLiveService.on("auction_chat", msg => {
  ...
  const teamPlayerSearchModal = document.getElementById("teamPlayerSearchModal");
  if (teamPlayerSearchModal) {
    teamPlayerSearchModal.addEventListener("click", e => {
      if (e.target === teamPlayerSearchModal) closeTeamPlayerSearchModal();
    });
  }

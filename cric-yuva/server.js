...
app.get("/api/chat/:roomType/:roomId", requireAuth, async(req,res)=>{try{const sql=await getDatabase(); const type=String(req.params.roomType).toUpperCase(); if(!["TEAM","TOURNAMENT","DIRECT","GROUP"].includes(type))return res.status(400).json({ok:false,error:"Invalid room"}); const rows=await sql`SELECT sender_id AS "userId",sender_name AS "senderName",message,created_at AS "createdAt" FROM chats WHERE room_type=${type} AND room_id=${req.params.roomId} ORDER BY id DESC LIMIT 100`; res.json({ok:true,success:true,messages:rows.reverse()});}catch(e){res.status(500).json({ok:false,error:e.message});}});
app.post("/api/chat/:roomType/:roomId", requireAuth, async(req,res)=>{try{const sql=await getDatabase(); const type=String(req.params.roomType).toUpperCase(); const message=String(req.body?.message||"").trim().slice(0,2000); if(!message||!["TEAM","TOURNAMENT","DIRECT","GROUP"].includes(type))return res.status(400).json({ok:false,error:"Invalid chat message"}); const r=await sql`INSERT INTO chats(room_type,room_id,sender_id,sender_name,message,created_at) VALUES(${type},${req.params.roomId},${req.user.userId},${req.user.name},${message},${nowMs()}) RETURNING id,sender_id AS "userId",sender_name AS "senderName",message,created_at AS "createdAt"`; broadcastRoom(type,req.params.roomId,{type:type+"_CHAT_MESSAGE",message:r[0]}); res.json({ok:true,success:true,message:r[0]});}catch(e){res.status(500).json({ok:false,error:e.message});}});
app.post("/api/chat/team/:tournamentId/:teamName", requireAuth, async(req,res)=>{req.params.roomType="TEAM"; req.params.roomId=`${req.params.tournamentId}:${decodeURIComponent(req.params.teamName)}`; return (async()=>{try{const sql=await getDatabase();const message=String(req.body?.message||"").trim().slice(0,2000);if(!message)return res.status(400).json({ok:false,error:"Invalid chat message"});const r=await sql`INSERT INTO chats(room_type,room_id,sender_id,sender_name,message,created_at) VALUES('TEAM',${req.params.roomId},${req.user.userId},${req.user.name},${message},${nowMs()}) RETURNING id,sender_id AS "userId",sender_name AS "senderName",message,created_at AS "createdAt"`;res.json({ok:true,success:true,message:r[0]});}catch(e){res.status(500).json({ok:false,error:e.message});}})();});
app.get("/api/chat/team/:tournamentId/:teamName", requireAuth, async(req,res)=>{req.params.roomType="TEAM";req.params.roomId=`${req.params.tournamentId}:${decodeURIComponent(req.params.teamName)}`;return app._router? (async()=>{try{const sql=await getDatabase();const rows=await sql`SELECT sender_id AS "userId",sender_name AS "senderName",message,created_at AS "createdAt" FROM chats WHERE room_type='TEAM' AND room_id=${req.params.roomId} ORDER BY id DESC LIMIT 100`;res.json({ok:true,success:true,messages:rows.reverse()});}catch(e){res.status(500).json({ok:false,error:e.message});}})():null;});
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", (raw) => {
      try {
        ...
        } else if ((msg.type === "TEAM_CHAT_MESSAGE" || msg.type === "TOURNAMENT_CHAT_MESSAGE") && (msg.teamId || msg.tournamentId || msg.tourneyId)) {
          const type = msg.teamId ? "TEAM" : "TOURNAMENT"; const rid = String(msg.teamId || msg.tournamentId || msg.tourneyId); broadcastRoom(type, rid, msg);
        }
      } catch (e) {}
    });
    ws.on("close", () => {

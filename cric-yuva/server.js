...
// Converts an older browser-only account into a central server account after
// the user proves the existing local credentials with the same mobile/password.
app.post("/api/auth/migrate-local", async (req, res) => {
  try {
    ...
    if (!/^\d{10,15}$/.test(cleanMobile)) {
      return res.status(400).json({ ok: false, success: false, error: "Valid mobile number required" });
    }
    if (!cleanName || String(password).length < 4) {
      return res.status(400).json({ ok: false, success: false, error: "Valid name and password are required" });
    }

    // If the account already exists centrally, never create another one.
    const existingUser = await sql`
      SELECT user_id, mobile, name, password_hash, player_id
      FROM users WHERE mobile = ${cleanMobile} LIMIT 1
    `;
    if (existingUser.length > 0) {
      const user = existingUser[0];
      if (!verifyPassword(password, user.password_hash)) {
        return res.status(409).json({ ok: false, success: false, error: "This mobile number already has a different central account. Please use that account." });
      }
      const now = Date.now();
      const token = crypto.randomBytes(32).toString("hex");
      const sessionExpiry = now + 30 * 24 * 60 * 60 * 1000;
      ...
      `;
      return res.json({ ok: true, success: true, migrated: false, token, user: {
        userId: user.user_id, mobile: user.mobile, name: user.name, playerId: user.player_id
      }});
    }

    // Reuse an orphan player if one exists for this mobile; otherwise create a new permanent ID.
    const existingPlayer = await sql`
      SELECT player_id, user_id, name
      FROM players WHERE mobile = ${cleanMobile}
      ORDER BY created_at ASC LIMIT 1
    `;
    if (existingPlayer.length > 0 && existingPlayer[0].user_id) {
      return res.status(409).json({ ok: false, success: false, error: "This mobile number is already linked to another player account." });
    }

    const playerId = existingPlayer.length > 0 && existingPlayer[0].player_id
      ? existingPlayer[0].player_id
      : await generatePermanentPlayerId(sql);
    const userId = "USR-" + Date.now() + "-" + crypto.randomBytes(4).toString("hex").toUpperCase();
    const passwordHash = hashPassword(password);
    const now = Date.now();
    const profileData = {
      name: cleanName, mobile: cleanMobile, playerId,
      role: "All-Rounder", jerseyNumber: 7
    };

    await sql.begin(async tx => {
      await tx`
        INSERT INTO users (user_id, mobile, name, password_hash, player_id, created_at)
        VALUES (${userId}, ${cleanMobile}, ${cleanName}, ${passwordHash}, ${playerId}, ${now})
      `;
      await tx`
        INSERT INTO profiles (user_id, data_json, updated_at)
        VALUES (${userId}, ${JSON.stringify(profileData)}::jsonb, ${now})
      ...
      if (existingPlayer.length > 0) {
        await tx`
          UPDATE players SET user_id = ${userId}, name = ${cleanName},
            data_json = ${JSON.stringify(profileData)}::jsonb, updated_at = ${now}
          WHERE player_id = ${playerId}
        `;
      } else {
        await tx`
          INSERT INTO players (player_id, user_id, name, mobile, role, jersey_number, data_json, created_at, updated_at)
          VALUES (${playerId}, ${userId}, ${cleanName}, ${cleanMobile}, 'All-Rounder', 7, ${JSON.stringify(profileData)}::jsonb, ${now}, ${now})
        `;
      }
      const token = crypto.randomBytes(32).toString("hex");
      const sessionExpiry = now + 30 * 24 * 60 * 60 * 1000;
      await tx`
        ...
        VALUES (${token}, ${userId}, ${now}, ${sessionExpiry})
      `;
      return tx`SELECT ${token}::text AS token`;
    }).then(async rows => {
      const token = rows[0]?.token;
      res.json({ ok: true, success: true, migrated: true, token, user: {
        userId, mobile: cleanMobile, name: cleanName, playerId
      }});
    });
  } catch (err) {
    console.error("Local account migration error:", err.message);
    res.status(500).json({ ok: false, success: false, error: "Could not migrate local account" });
  }
});
